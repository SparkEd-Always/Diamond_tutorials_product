...
INVALID_OTP=$(api_call_json "POST" "/mobile/auth/send-otp" "{\"phone_number\":\"9999999999\"}")
ERROR_MSG=$(echo "$INVALID_OTP" | jq -r '.detail // empty')

if echo "$ERROR_MSG" | grep -qi "not registered\|not found\|pattern"; then
...
NO_AUTH_RESPONSE=$(curl -s -X GET "${API_BASE}/students/")
...
STUDENTS_RESPONSE=$(curl -s -X GET "${API_BASE}/students/" -H "Authorization: Bearer $ADMIN_TOKEN")
...
SINGLE_STUDENT=$(curl -s -X GET "${API_BASE}/students/1" -H "Authorization: Bearer $ADMIN_TOKEN")
...
# Test 4.2: Create Communication (using send-bulk endpoint)
echo "Test 4.2: Create Communication"
COMM_RESPONSE=$(curl -s -X POST "${API_BASE}/communications/send-bulk" \
    -H "Authorization: Bearer $ADMIN_TOKEN" \
    -H "Content-Type: application/json" \
    -d '{"subject":"Test Message","message":"Automated test message","recipients":"all_parents"}')

# Check if request was accepted (may return success or sent count)
if echo "$COMM_RESPONSE" | jq -e '.success // .sent // .message' > /dev/null 2>&1; then
    log_test "Test 4.2: Create Communication" "PASS" "Message created/queued"
else
...
if echo "$INVALID_TOKEN_ERROR" | grep -qi "not authenticated\|invalid\|decode\|credentials"; then
